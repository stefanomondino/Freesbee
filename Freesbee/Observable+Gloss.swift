...
public extension ObservableType where E == Response {
    ...
    public func mapObject<T: Decodable>(type: T.Type) -> Observable<T> {
        return flatMap { response -> Observable<T> in
            return Observable.just(try response.filterSuccessfulStatusAndRedirectCodes().mapObject(T.self))
        }
    }
    ...
    public func mapObject<T: Decodable>(type: T.Type, forKeyPath keyPath: String) -> Observable<T> {
        return flatMap { response -> Observable<T> in
            return Observable.just(try response.filterSuccessfulStatusAndRedirectCodes().mapObject(T.self, forKeyPath: keyPath))
        }
    }
    ...
    public func mapArray<T: Decodable>(type: T.Type) -> Observable<[T]> {
        return flatMap { (response) -> Observable<[T]> in
            return Observable.just(try response.filterSuccessfulStatusAndRedirectCodes().mapArray(T.self))
        }
    }
    ...
    public func mapArray<T: Decodable>(type: T.Type, forKeyPath keyPath: String) -> Observable<[T]> {
        return flatMap { (response) -> Observable<[T]> in
            return Observable.just(try response.filterSuccessfulStatusAndRedirectCodes().mapArray(T.self, forKeyPath: keyPath))
        }
    }
}
